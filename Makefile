...
QA_DOCKER_IMAGE=jakzal/phpqa:1.19-${BUILD_ENV}-alpine
...
phpunit-coverage: ensure
	sh -c "${QA_DOCKER_COMMAND} phpdbg -qrr /tools/phpunit-7 --verbose --coverage-text --log-junit=var/phpunit.junit.xml --coverage-xml var/coverage-xml/"

infection: phpunit-coverage
	sh -c "${QA_DOCKER_COMMAND} phpdbg -qrr /tools/infection run --verbose --show-mutations --no-interaction --only-covered --coverage var/ --min-msi=100 --min-covered-msi=100 --threads 4"
